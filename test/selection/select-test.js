...
var suite = vows.describe("d4.select");
  ...
  "select": {
    ...
    "on a simple page": {
      ...
      "sets the parentNode to the document element": function(d4) {
        var selection = d4.select("body"),
            document = d4.selection().node().ownerDocument;
        assert.strictEqual(selection[0].parentNode, document.documentElement);
      },
      "does not propagate data from the document": function(d4) {
        var document = d4.selection().node().ownerDocument;
        document.__data__ = 42;
        delete document.body.__data__;
        try {
          assert.isUndefined(d4.select("body").datum());
        } finally {
          delete document.__data__;
        }
      },
      "does not propagate data from the document element": function(d4) {
        var document = d4.selection().node().ownerDocument;
